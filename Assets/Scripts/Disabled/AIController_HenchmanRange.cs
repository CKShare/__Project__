...
public class AIController_HenchmanRange : ControllerBase
{
    ...
    private Vector2 _attackDelayMinMax;
    [SerializeField]
    private GunAttacker _gunAttacker = new GunAttacker();
    ...
    private AIState_HenchmanRange _currentState = AIState_HenchmanRange.None;
    ...
    protected override void Start()
    {
        ...
        ChangeState(AIState_HenchmanRange.Idle);
    }
    ...
    private void ChangeState(AIState_HenchmanRange state)
    {
        if (_currentState != AIState_HenchmanRange.None)
    ...
    private void OnStateEnter(AIState_HenchmanRange state)
    {
        switch (state)
        {
            case AIState_HenchmanRange.Idle:
                {
                    if (_patrolOnAwake)
                    {
                        ChangeState(AIState_HenchmanRange.Patrol);
                    }
                }
                break;

            case AIState_HenchmanRange.Patrol:
                {
            ...
            case AIState_HenchmanRange.Combat:
                {
                    _attackDelay = Random.Range(_attackDelayMinMax.x, _attackDelayMinMax.y);
            ...
            case AIState_HenchmanRange.Alert:
                {
                    _missingElapsedTime = 0F;
            ...
            case AIState_HenchmanRange.Hit:
                {
                    _pathfinder.canMove = _pathfinder.canSearch = false;
                    _isHitting = true;
            ...
            case AIState_HenchmanRange.Dead:
                {
                    _pathfinder.canMove = _pathfinder.canSearch = false;
    ...
    private void OnStateUpdate(AIState_HenchmanRange state)
    {
        switch (state)
        {
            case AIState_HenchmanRange.Idle:
                {
                    ...
                    {
                        ChangeState(AIState_HenchmanRange.Combat);
                    }
                }
                break;
            ...
            case AIState_HenchmanRange.Patrol:
                {
                    ...
                    else
                    {
                        if (!_pathfinder.pathPending && _pathfinder.reachedEndOfPath)
                        {
                            ...
                            ChangeState(AIState_HenchmanRange.Alert);
                        }
                    }
                }
                break;
            ...
            case AIState_HenchmanRange.Combat:
                {
                    ...
                    else
                    {
                        ...
                        if (_attackElapsedTime >= _attackDelay)
                        {
                            ...
                            _attackDelay = Random.Range(_attackDelayMinMax.x, _attackDelayMinMax.y);
                            _gunAttacker.Attack(Transform, _targetCollider.bounds.center + new Vector3(0F, _targetCollider.bounds.extents.y * 0.5F, 0F));
    ...
    private void OnStateExit(AIState_HenchmanRange state)
    {
        switch (state)
        {
            case AIState_HenchmanRange.Patrol:
    ...
    public override void ApplyDamage(Transform attacker, int damage, int reactionID)
    {
        ...
        ChangeState(AIState_HenchmanRange.Hit);
    ...
    protected override void OnDeath()
    {
        ...
        ChangeState(AIState_HenchmanRange.Dead);
    }
